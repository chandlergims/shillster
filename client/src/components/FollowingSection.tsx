...
const getImageUrl = (path: string, baseUrl: string) => {
  ...
  if (path.startsWith('/uploads')) {
    ...
    const baseDomain = baseUrl.endsWith('/api') 
      ? baseUrl.substring(0, baseUrl.length - 4) // Remove /api
      : baseUrl;
  ...
  return (
    <div className="col-span-3 bg-[#1b1d22] border border-[#282b33] p-4 rounded-xl">
      <div className="flex items-center mb-4">
        ...
        <span className="ml-auto bg-[#24272e] text-[#97ef83] px-2 py-0.5 text-sm font-bold rounded-md">{followingUsers.length}</span>
      </div>
